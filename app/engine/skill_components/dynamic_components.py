...
class DynamicEarlyAttacks(SkillComponent):
    nid = 'dynamic_early_attacks'
    desc = "Gives +X extra phases before the foe can counterattack"
    tag = SkillTags.DYNAMIC

    expose = ComponentType.String

    def dynamic_early_attacks(self, unit, item, target, item2, mode, attack_info, base_value) -> int:
        from app.engine import evaluate
        try:
            local_args = {'item': item, 'item2': item2, 'mode': mode, 'skill': self.skill, 'attack_info': attack_info, 'base_value': base_value}
            return int(evaluate.evaluate(self.value, unit, target, unit.position, local_args))
